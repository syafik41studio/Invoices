  ...
  $.fn.tokenField = function(options) {
    ...
    return this.each(function(){
      ...
      var field = $('<div></div>')
        ...
        .replaceAll(this);
      if (!settings.nested)
        if(name == 'web_session[tag_list]'){
            field.prepend("<input type='hidden' id=web_session_course value='"+tokens.join(',')+"' name='"+name+"'/>")
        }else{
            field.prepend("<input type='hidden' id='"+name+"' value='"+tokens.join(',')+"' name='"+name+"'/>")
        }
